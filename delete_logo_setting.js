...
let db = new sqlite3.Database(DATABASE_PATH, (err) => {
    if (err) {
        ...
        db.run(`DELETE FROM settings WHERE key = 'landing_logo'`, function(err) {
            if (err) {
                console.error("Error deleting setting:", err.message);
            } else {
                console.log(`Setting 'landing_logo' deleted successfully (if it existed). Rows affected: ${this.changes}`);
            }
            db.close((err) => {
                if (err) {
                    console.error(err.message);
